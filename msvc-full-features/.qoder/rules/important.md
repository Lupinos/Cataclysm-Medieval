...
为避免进度文档因琐碎记录和冗长琐事变得难以阅读，每次更新 `docs/process/` 目录下的文档时，必须遵循以下规则：
1. **屏蔽细碎小错误**：诸如单词拼写、格式微调、调试过程中的单次 Gotchas（例如字段缺失纠正、单次 L0 校验等），在修复后**直接闭环即可，严禁记录进 process 进度文档中**。
2. **物品类 process 开头必须设物品索引 (Index)**：针对物品/装备类的实现进度文档，**开头部分必须放置一份结构化的完整物品状态总览表格 (Index)**，列出当前系统所有已做和将做物品项（类似 Walkthrough 格式），后续的其他章节细节以此表格的 ID 和清单进行索引和扩展。
3. **结构聚焦与一目了然**：Process 文档应该保持精炼、一目了然。核心目标是使用户打开文档就能立刻清晰看到：“目前该系统有哪些东西已经做好了，有哪些东西还没做（通过带状态的 checklist 清晰呈现）”。
